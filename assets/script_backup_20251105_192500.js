// ======================================
// ðŸ’ Bijoux Project v2.1 â€“ Novembre 2025
// Modale produit + Filtre "evenement" + Panier flottant animÃ©
// Remplace entiÃ¨rement ton script.js actuel
// ======================================
...
// const CART_PAGE = "/assets/cart.html"; // âŒ supprimÃ© car le panier est intÃ©grÃ© dans la page
...
/* ---------------------------
   UTIL: parseCSVLine
   (gÃ¨re champs entre guillemets et virgules internes)
   --------------------------- */
...
/* ---------------------------
   ...
   - dÃ©tecte images supplÃ©mentaires
   - prÃ©pare 'evenement' pour futur filtre
   --------------------------- */
function normalizeProduct(rawObj) {
  ...
  const rawPriceStr = get(["price_euros", "price â‚¬", "price", "prix", "priceâ‚¬"]) || "0";
  const cleanPrice = rawPriceStr.replace(/[^\d,.\-]/g, "").replace(",", ".");
  const prix = parseFloat(cleanPrice) || 0;

  const ref = get(["reference", "rÃ©fÃ©rence", "ref"]) || "";
  const imageField = get(["image", "images", "photo", "photos"]) || "";

  const autresImagesRaw = get(["images_supplÃ©mentaires", "images_supplementaires", "vues", "photos_supp", "images"]);
  ...
  const qteTailleRaw = get([
    ...
    "quantitÃ©_par_taille",
    "quantitÃƒÂ©_ par_ taille",
    "quantite_par_taille",
    "quantite par taille",
    "qte_par_taille",
    "qte_taille"
  ...
  const normalized = {
    ...
    evenement: get(["evenement", "Ã©vÃ¨nement", "event"])
  };
...
/* ---------------------------
   RENDER PRODUCTS
   - affiche les cartes produits Ã  partir du tableau `products`
   - clic sur une image â†’ ouvre la modale dÃ©taillÃ©e
   --------------------------- */
function renderProducts(products) {
  const container = document.querySelector("#product-list");
  if (!container) return;

  // ðŸ§© Si aucun produit Ã  afficher
  if (!products || products.length === 0) {
    container.innerHTML = "<p>Aucun bijou trouvÃ© ðŸ•Šï¸</p>";
    return;
  }

  // ðŸ–¼ï¸ Construction des cartes produits
  ...
  // ðŸ–±ï¸ Gestion du clic sur chaque carte â†’ ouverture modale
  ...
  cards.forEach((card, index) => {
    card.addEventListener("click", () => {
      ...
      if (produit) {
        ...
        console.warn("âš ï¸ Produit introuvable Ã  lâ€™index", index);
      }
    });
  });
}
...
/* ---------------------------
   âœ… Ouvre la modale produit dÃ©taillÃ©e
   --------------------------- */
function openProductModal(product) {
  ...
  modal.innerHTML = `
    <div class="modal-content">
      ...
      <div class="modal-right">
        ...
        <p class="ref">RÃ©f : ${escapeHtml(product.reference)}</p>
        <p class="desc">${escapeHtml(product.description || "Aucune description")}</p>

        <p class="price">${product.prix.toFixed(2)} â‚¬</p>
        ...
        <button class="add-to-cart-btn">ðŸ›’ Ajouter au panier</button>
      </div>
    </div>
  `;
  ...
  // Miniatures â†’ changent lâ€™image principale
...
/* ---------------------------
   renderSizesHtml: gÃ©nÃ¨re HTML pour tailles + boutons dans la modale
...
/* ---------------------------
   animateCartIcon: rebond + halo dorÃ©
   --------------------------- */
...
/* ---------------------------
   ensureCartIcon: crÃ©e l'icÃ´ne panier flottante si absente
   - renvoie l'Ã©lÃ©ment
   --------------------------- */

  icon.className = "cart-icon";
  icon.innerHTML = `ðŸ›’<div id="cartCount" class="cart-count">0</div>`;
  document.body.appendChild(icon);

  // Petit fallback CSS au cas oÃ¹ le style manque
  ...
  // âœ… Nouveau comportement : ouvrir / fermer le panneau intÃ©grÃ©
  icon.addEventListener("click", () => {
    const panel = document.getElementById("cart-panel");
    if (panel) {
      panel.classList.toggle("open"); // bascule ouverture/fermeture
    } else {
      console.warn("âš ï¸ Aucun panneau panier (#cart-panel) trouvÃ© dans le DOM.");
    }
    /* ---------------------------
   ensureCartIcon: crÃ©e l'icÃ´ne panier flottante si absente
   - renvoie l'Ã©lÃ©ment
...
function ensureCartIcon() {
  ...
  // CrÃ©e lâ€™icÃ´ne panier flottante
 ...
 // âœ… Nouvelle version : ouvre le panneau intÃ©grÃ© au lieu de rediriger
icon.addEventListener("click", () => {
  console.log("ðŸ›’ IcÃ´ne panier cliquÃ©e !");
  const panel = document.getElementById("cart-panel");
  if (panel) {
    panel.classList.toggle("open");
    console.log("ðŸ”„ Ã‰tat du panneau :", panel.classList);
  } else {
    console.warn("âš ï¸ Aucun panneau panier (#cart-panel) trouvÃ© dans le DOM.");
  }
});
...
/* ---------------------------
   openFabricationPage: redirection prÃ©-remplie
   --------------------------- */
...
function populateFilters() {
  ...
  // evenement: supporte plusieurs evenements par produit sÃ©parÃ©s par , or ;
  const allEvents = allProducts.flatMap(p => ((p.evenement || "").split(/[,;]+/).map(s=>s.trim()).filter(Boolean)));
  fillSelect("#eventFilter", uniqueValues(allEvents), "Ã‰vÃ¨nement");
...
function fillSelect(selector, values, label) {
  ...
  if (!el) return;
  const first = el.querySelector("option")?.outerHTML || `<option value="">â€” ${label} â€”</option>`;
...
async function loadProducts() {
  try {
    ...
    if (!resp.ok) throw new Error("Ã‰chec du chargement CSV");
...
function setupEvents() {
  ...
  // ===========================
// âœ… Gestion de l'icÃ´ne panier (ouvre le panneau intÃ©grÃ©)
// ===========================
function ensureCartIcon() {
  ...
  icon.addEventListener("click", () => {
    const panel = document.getElementById("cart-panel");
    if (panel) {
      panel.classList.add("open"); // Ouvre le panneau intÃ©grÃ©
    } else {
      console.warn("âš ï¸ Aucun panneau panier (#cart-panel) trouvÃ© dans le DOM.");
    }
  });
...
document.addEventListener("DOMContentLoaded", async () => {
  await loadProducts();
  setupEvents();
  initEventListeners(); // â¬…ï¸ On ajoute l'initialisation Stripe ici
  ensureCartIcon(); // â¬…ï¸ On appelle ici
});


// ===========================
// âœ… Fonction pour envoyer le panier Ã  Stripe et rediriger vers le paiement
// ===========================
const checkoutWithStripe = async (cart) => {
  try {
    ...
    if (session.url) {
      ...
      showTempMessage("Erreur de crÃ©ation de session Stripe.", 3000);
    }
  } catch (err) {
    console.error("Erreur Stripe :", err);
    showTempMessage("Une erreur est survenue, veuillez rÃ©essayer.", 3000);
  }
};

// ===========================
// âœ… Fonction dâ€™initialisation du bouton Â« Payer avec Stripe Â»
// ===========================
const initEventListeners = () => {
  const checkoutBtn = document.getElementById("checkout-btn");
  if (checkoutBtn) {
    checkoutBtn.addEventListener("click", async () => {
      const cart = getCart(); // RÃ©cupÃ¨re le panier depuis localStorage
      if (!cart || cart.length === 0) {
        showTempMessage("Votre panier est vide ðŸ•Šï¸", 2000);
        return;
      }
...
// ===========================
// âœ… Gestion du panneau panier (ouverture / fermeture)
// ===========================
document.addEventListener("DOMContentLoaded", () => {
  ...
  // âœ… Gestion de l'icÃ´ne panier (ouvre le panneau intÃ©grÃ©)
  // ===========================
  const icon = document.getElementById("cart-icon"); // âœ… assure qu'on vise bien ton icÃ´ne
  if (icon) {
    icon.addEventListener("click", () => {
      const panel = document.getElementById("cart-panel");
      if (panel) {
...
/* ---------------------------
   ...
   - gÃ¨re les doublons par rÃ©fÃ©rence + taille
   - met Ã  jour l'icÃ´ne panier
   --------------------------- */
function addToCart(product, selectedSize = "") {
  if (!product) return;

  // RÃ©cupÃ¨re le panier existant
  let cart = JSON.parse(localStorage.getItem("cart") || "[]");

  // VÃ©rifie si le produit (mÃªme rÃ©f + taille) est dÃ©jÃ  dans le panier
  ...
  if (existingIndex !== -1) {
    // DÃ©jÃ  prÃ©sent â†’ incrÃ©mente la quantitÃ©
    cart[existingIndex].quantite += 1;
  } else {
    // Sinon â†’ ajoute un nouvel objet
    cart.push({
  ...
  showTempMessage("âœ… AjoutÃ© au panier !", 1500);
}

/* ---------------------------
   âœ… Fonctions panier complÃ©mentaires
   --------------------------- */
function getCart() {
