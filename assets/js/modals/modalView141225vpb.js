...
export class ProductModal extends ModalBase {
    ...
    // ----------------------------
    const sizeOptions = product.available_sizes.length
        ? product.available_sizes.map(size => {
              const qty = product.stock_by_size?.[size] ?? 0;
        ...
        <div class="modal-product-grid">
            ...
            <div class="modal-left">
                <img class="modal-main-img"
                     ...
                     onerror="this.src='${window.CONFIG?.placeholder || ''}'">
            </div>

            <!-- INFOS -->
            <div class="modal-right">

                <h2 class="modal-title">${product.titleLabel}</h2>

                <div class="modal-price">
                    ${product.price.toFixed(2).replace(".", ",")} €
                </div>

                ${product.type ? `<div class="modal-type">${product.type}</div>` : ""}
                ...
                <ul class="modal-details">
                    <li><strong>Métal :</strong> ${product.metalLabel}</li>
                    <li><strong>Couleur :</strong> ${product.colorLabel || "—"}</li>
                    <li><strong>Poids or :</strong>
                        ${product.metal_weight ? product.metal_weight + " g" : "—"}
                    </li>
                    <li><strong>Pierre :</strong>
                        ${product.stone_type || "Sans pierre"}
                        ${product.stone_weight ? " • " + product.stone_weight + " ct" : ""}
                    </li>
                </ul>
                ...
                ${
                    product.available_sizes.length
                        ...
                        `
                        : `<p class="modal-sizes">Taille unique</p>`
                }
...
extractProductData(product) {
    ...
    if (sizeSelect && addBtn) {
        // Désactiver le bouton par défaut s'il y a un sélecteur de taille
        addBtn.disabled = true;
        
        // Si le produit a une taille unique, activer directement le bouton
        if (product.available_sizes && product.available_sizes.length === 1) {
            sizeSelect.value = product.available_sizes[0];
...
handleAddToCart(product) {
    ...
    if (window.addToCart) {
        // Passer la taille sélectionnée si elle existe
